...
function listPosts(string $postPath): string {
    ...
    $blog = fetchSubEntries($root_path . $postPath, $filter, $sorting, PAGE_SUB_BLOG);
    $total_posts = count($blog['sub-items']) ?? 0;

    if ($total_posts === 0) {
        $out .= "<p>(".$txt['no-posts'].")</p>\n";
        return $out;
    }
    ...
    if (!empty($filterDesc)) {
        ...
        $out .= "<p>".$txt['show']." <strong>{$showing_from}–{$showing_to}</strong> (".$txt['in']." {$langName}).</p>\n";
    }
    ...
    foreach ($posts_to_show as $entry) {
        ...
        $out .= "(" . $date . ")\n";
        $out .= "<br/>" . $entry['abstract'] . "</p>\n\n";
    }

    $out .= "<p>".$txt['show']." {$showing_from}–{$showing_to} ".$txt['of']." {$total_posts}</p>\n";
    $out .= "<p>";

    if (!$show_all) {
        if ($start_from > 0) {
            $prev_start = max(0, $start_from - $num_posts);
            $out .= "<a href=\"?{$filter_query}&StartFrom={$prev_start}&NumberPosts={$num_posts}\">← ".$txt['prev']."</a> ";
        }

        if ($showing_to < $total_posts) {
            $next_start = $start_from + $num_posts;
            $out .= "<a href=\"?{$filter_query}&StartFrom={$next_start}&NumberPosts={$num_posts}\">".$txt['next']." →</a> ";
        }

        $out .= "<a href=\"?{$filter_query}&StartFrom=0&NumberPosts=0\">".$txt['all']."</a>";
        $out .= (!empty($clearfilters)) ? " / ".$clearfilters."\n" : "\n";
    } else {
        $out .= "<a href=\"?{$filter_query}\">".$txt['less']."</a>";
        $out .= (!empty($clearfilters)) ? " / ".$clearfilters."\n" : "\n";
    }
